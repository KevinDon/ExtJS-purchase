ReportProductAnalysisForm = Ext.extend(HP.Window, {
	constructor : function(_cfg) {
		...
		var conf = {
			title : _lang.Reports.mTitle + $getTitleSuffix(this.action),
			winId : 'ReportProductAnalysisFormWinID',
		...
		ReportProductAnalysisForm.superclass.constructor.call(this, {
			id : conf.winId,
			scope : this,
			title : conf.title,
    ...
    saveRow: function(){
        ...
        if(rows.length>0){
            for(index in rows){
                params['products['+index+'].reportsId'] = businessId;
