ReportProductComplianceView = Ext.extend(Ext.Panel, {
	constructor: function(conf) {
		Ext.applyIf(this, conf);

		var conf = {
			title : _lang.ReportProductCompliance.mTitle,
            ...
            urlList: __ctxPath + 'reportproductcompliance/list',
			urlSave: __ctxPath + 'reportproductcompliance/save',
			urlDelete: __ctxPath + 'reportproductcompliance/delete',
		...
		ReportProductComplianceView.superclass.constructor.call(this, {
	...
	initUIComponents: function(conf) {		
		...
		this.gridPanel = new HP.GridPanel({
			...
			title: _lang.ReportProductCompliance.tabListTitle,
			collapsible: true,
			split: true,
			scope: this,
			forceFit: false,
			url: conf.urlList,
			fields: [
				'id','title','vendorId','vendorCnName','vendorEnName','vendorId','businessId','serialNumber', 'reportTime','confirmedEnName','confirmedCnName','confirmedId',
		...
		this.viewPanel = new HP.FormPanel({
			...
			fieldItems: [
                ...
                { xtype: 'container', cls:'row',  items:[
                    {field: 'main.compliance.productProblem', xtype: 'displayfield', width:'100%', cls:'col-1'},
                ]},

                { xtype: 'section', title: _lang.Reports.fStandard},
                { xtype: 'container', cls:'row',  items:[
                    {field: 'main.compliance.standard', xtype: 'displayfield', width:'100%', cls:'col-1'},
                ]},

                { xtype: 'section', title: _lang.Reports.fEssentialStandard},
                { xtype: 'container', cls:'row',  items:[
                    {field: 'compliance.essentialStandard', xtype: 'displayfield', width:'100%', cls:'col-1'},
                ]},

                { xtype: 'section', title: _lang.Reports.fEvaluationSafetyRecommendations},
                { xtype: 'container', cls:'row',  items:[
                    {field: 'main.compliance.evaluationSafetyRecommendations', xtype: 'displayfield', width:'100%', cls:'col-1'},
                ]},
            ...
            preName: 'main', loadMask:true, maskTo: conf.mainViewPanelId, success:function(response){
               ...
               if(!!json.data && !!json.data.details && json.data.details.length>0){
                   for(var index in json.data.details){
                       json.data.details[index].product.prop.riskRating = json.data.details[index].cplRiskRating;
                   }
               };
    ...
    editRow: function(conf){
        new ReportProductComplianceForm(conf).show();
    }
});