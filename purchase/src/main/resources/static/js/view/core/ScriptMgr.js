...
ScriptLoader.prototype = {
	...
	processSuccess : function(response) {
		if (typeof response.request.options.argument.callback == 'function') {
			response.request.options.argument.callback.call(response.request.options.argument.scope,
			response.responseText,response.request.options.argument.url,response.request.options.argument.index);
		}
	},
	...
	processFailure : function(response) {
		Ext.MessageBox.show({
					title : '应用程序出错',
					msg : 'Js脚本库加载出错，服务器可能停止，请联系管理员。文件路径：' + response.request.options.argument.url,
					closable : false,
