...
@RestController
@RequestMapping("/reportsupplierinvestigation")
public class SupplierInvestigationController extends ControllerBase{
    ...
    @RequiresPermissions("ReportSupplierInvestigation:normal:list")
    @PostMapping("/list")
    public RestResult list(ServletRequest request, String sort, String keywords,
    	...
    	try {
			...
			params = setParams(params, "ReportSupplierInvestigation", ":4:3:2:1", false);

			params.put("moduleName-S-EQ-ADD", "ReportSupplierInvestigation");
    ...
    @RequiresPermissions(value = {"ReportSupplierInvestigation:normal:add","ReportSupplierInvestigation:normal:edit"}, logical = Logical.OR)
    @PostMapping("/save")
	public RestResult save(String act, @ModelAttribute("main") Reports main) {
		RestResult result = new RestResult();
		try {
			...
			main.setModuleName("ReportSupplierInvestigation");
			if( main.getId() != null && StringUtils.isNotBlank(main.getId())){
				//复制另存
				if(StringUtils.isNotBlank(act) && ACT_COPY.equals(act)){
					reportsService.saveAs(main, null);
				}else{
					reportsService.save(main, null);
				}
    ...
    @RequiresPermissions("ReportSupplierInvestigation:normal:del")
	@PostMapping("/delete")
	public RestResult delete(String ids) {
		...
		try {
			...
			if(hasDataType("ReportSupplierInvestigation" + ":4")){
				//物理删除
