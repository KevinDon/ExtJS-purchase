...
@RestController
@RequestMapping("/reportsproductanalysis")
public class ProductAnalysisController extends ControllerBase{
    ...
    @RequiresPermissions("ReportProductAnalysis:normal:list")
    @PostMapping("/list")
    ...
    @RequiresPermissions(value = {"ReportProductAnalysis:normal:add","ReportProductAnalysis:normal:edit"}, logical = Logical.OR)
    @PostMapping("/save")
	public RestResult save(String act, @ModelAttribute("main") Reports main, ReportsProductsVo products) {
		RestResult result = new RestResult();
		try {

			main.setModuleName("ReportProductAnalysis");
    ...
    @RequiresPermissions("ReportProductAnalysis:normal:del")
	@PostMapping("/delete")
	public RestResult delete(String ids) {
		...
		try {
			...
			if(hasDataType("ReportProductAnalysis" + ":4")){
				//物理删除
