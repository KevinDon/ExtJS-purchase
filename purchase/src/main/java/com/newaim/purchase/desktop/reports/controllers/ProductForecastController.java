...
@RestController
@RequestMapping("/reportproductforecast")
public class ProductForecastController extends ControllerBase{
    ...
    @RequiresPermissions("ReportProductForecast:normal:list")
    @PostMapping("/list")
    public RestResult list(ServletRequest request, String sort, String keywords,
    	...
    	try {
			...
			params = setParams(params, "ReportProductForecast", ":4:3:2:1", false);

			params.put("moduleName-S-EQ-ADD", "ReportProductForecast");
    ...
    @RequiresPermissions(value = {"ReportProductForecast:normal:add","ReportProductForecast:normal:edit"}, logical = Logical.OR)
    @PostMapping("/save")
	public RestResult save(String act, @ModelAttribute("main") Reports main, ReportsProductsVo products) {
		...
		try {
			...
			main.setModuleName("ReportProductForecast");
    ...
    @RequiresPermissions("ReportProductForecast:normal:del")
	@PostMapping("/delete")
	public RestResult delete(String ids) {
		...
		try {
			...
			if(hasDataType("ReportProductForecast" + ":4")){
				//物理删除
