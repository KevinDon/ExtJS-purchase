...
public class JsonMapper {
     ...
     * @see #fromJson(String, JavaType)
     */
    public <T> T fromJson(String jsonString, Class<T> clazz, Class<?>... classes) {
        if (StringUtils.isEmpty(jsonString)) {
            return null;
        }
        try {
            if(classes != null && classes.length > 0){
                JavaType[] javaTypes = new JavaType[classes.length];
                for (int i = 0; i < classes.length; i++) {
                    javaTypes[i] = buildCollectionType(List.class, classes[i]);
                }
                JavaType javaType = constructParametricType(clazz, javaTypes);
     ...
     * 构造Collection类型.
     */
    public JavaType buildCollectionType(Class<? extends Collection> collectionClass, JavaType javaType) {
