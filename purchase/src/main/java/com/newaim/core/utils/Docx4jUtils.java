...
//    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        String realPath ="e://FlowNewProductViewFormPanelID.xml";
        String outputFilePath ="e://FlowNewProductViewFormPanelID.docx";

//        InputStream is = new FileInputStream("e://FlowNewProductViewFormPanelID.docx");
//        XWPFDocument doc = new XWPFDocument(is);

//        XWPFDocument doc = new XWPFDocument(POIXMLDocument.openPackage(realPath));
//        OutputStream os = new FileOutputStream("e://s.docx");
//        doc.write(os);
//        os.close();
        //转换对象
        Map<String, Object> params = Maps.newHashMap();
        params.put("id", "new-test-001");

        //模板替换
        Configuration config = new Configuration();
        config.setDirectoryForTemplateLoading(new File("e://"));
        Template template = config.getTemplate("/FlowNewProductViewFormPanelID.xml","UTF-8");
        template.setEncoding("utf-8");
        StringWriter out = new StringWriter();
        template.process(params, out);
        String a =out.getBuffer().toString();
        out.flush();
        out.close();
        FileUtil.write(realPath + "-new.xml", a, true, true);

        WordprocessingMLPackage wordMLPackage = WordprocessingMLPackage.load(new File(realPath + "-new.xml"));
